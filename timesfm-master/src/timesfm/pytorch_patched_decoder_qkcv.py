...
class TimesFMAttention(nn.Module):
  ...
  def forward(
    ...
    ### qkcv
    # print('\r', f'[TimesFMAttention] before _projection, queries.shape = {q.shape}, ca.shape = {ca.shape if ca != None else None}, keys.shape = {k.shape} then {k.transpose(2, 3).shape}, v.shape = {v.shape}', end = '\n', flush=False)

    B, H, S, D = k.shape
    ca= ca.unsqueeze(1).expand(-1, S,-1,-1).reshape(B, H, S, D)
    ...
    # v2
        ...
        k = torch.mul(k, torch.sigmoid(ca))
...
class PatchedTimeSeriesDecoder_qkcv(nn.Module):
  """wapper for Patched time-series decoder."""

  def __init__(self, config: TimesFMConfig, 
               list_static_feature, 
               v_qkcv=-1,
               input_patch_len=7,
               input_horizon_len=28,
               use_patch_interpolate=2,
               embedding_type=0,
               input_embedding_type=0,
               ):
    """Initializes the PatchedTimeSeriesDecoder_qkcv class.
    Args:
      config: The configuration for the model.
      list_static_feature: List of static features.
      v_qkcv: The type of qkcv to use.
      input_patch_len: The length of the input patch.
      input_horizon_len: The length of the input horizon.
      use_patch_interpolate: Whether to use patch interpolation. 1: static interpolate, 2: linear transform, 3: train patching ResidualBlocks
      embedding_type: The type of embedding to use.
    """

    print(f"[PatchedTimeSeriesDecoder_qkcv] config: {config}")
    super().__init__()
    self.config = config

    ### qkcv
    self.v_qkcv = v_qkcv
    self.list_static_feature = list_static_feature
    self.c_Generator = C_Generator(config.head_dim * config.num_heads, len(list_static_feature), embedding_type=embedding_type, dropout=0.1)
    self.input_patch_len = input_patch_len
    self.input_embedding_type = input_embedding_type

    if self.input_embedding_type > 0:
      self.c_Generator_input = C_Generator(1, 
                                           1+len(list_static_feature), embedding_type=2-self.input_embedding_type, 
                                           dropout=0.01)

    self.use_patch_interpolate = use_patch_interpolate

    ### controlled input/output size
    self.input_horizon_len = input_horizon_len

    if self.use_patch_interpolate==2:
      self.linear_transform_patch_preprocess = nn.Linear(input_patch_len*2, 64)
      self.linear_transform_patch_postprocess = nn.Linear(128, self.input_horizon_len)

    elif self.use_patch_interpolate==3:
      self.linear_transform_patch_preprocess = ResidualBlock(
          input_dims=2 * self.input_patch_len,
          output_dims=config.hidden_size,
          hidden_dims=config.intermediate_size,
      )
      self.linear_transform_patch_postprocess = ResidualBlock(
        input_dims=config.hidden_size, # 1280
        output_dims=self.input_horizon_len * (1 + len(config.quantiles)),
        hidden_dims=config.intermediate_size,
    )
    self.patchedTimeSeriesDecoder = PatchedTimeSeriesDecoder(config)
  ...
  def forward(
    ...
    ### qkcv
    x_context_features, y_context, _, _, static_features = input_ts

    # print(f'[PatchedTimeSeriesDecoder_qkcv] x_context_features.shape = {x_context_features.shape}, y_context.shape = {y_context.shape}, static_features.shape = {static_features.shape}, self.config {self.config}', end = '\n', flush=False)

    if len(self.list_static_feature) != static_features.shape[-1]:
      raise ValueError(f"[PatchedTimeSeriesDecoder_qkcv] Static features length mismatch: {self.list_static_feature} vs {static_features.shape[-1]}")

    ca = self.c_Generator(static_features)

    if self.input_embedding_type>0:
      # print(f"[PatchedTimeSeriesDecoder_qkcv] forward() static_features {static_features.shape}, x_context_features {x_context_features.shape}, y_context {y_context.shape}", end = '\n', flush=True) 
      C = y_context.shape[-1]
      y_context = torch.cat((static_features.expand(-1, C,-1), y_context.unsqueeze(-1)), dim=-1)
      y_context = self.c_Generator_input(y_context)[:,:,0]

    output_ts, scores = self.patchedTimeSeriesDecoder(
      y_context, 
      input_padding, 
      freq, 
      self.use_patch_interpolate,
      self.linear_transform_patch_preprocess, 
      self.linear_transform_patch_postprocess,
      ca=ca, v_qkcv=self.v_qkcv,
      input_horizon_len=self.input_horizon_len,
      )
    return output_ts, ca, scores
  ...
  def decode(
    ...
    full_outputs=self.patchedTimeSeriesDecoder.decode(
      input_ts,
      paddings,
      freq,
      horizon_len,
...
class PatchedTimeSeriesDecoder(nn.Module):
  ...
  def __init__(self, config: TimesFMConfig):
    print(f"[PatchedTimeSeriesDecoder]config: {config}")
    super().__init__()
    self.config = config

    # Linear transformation for the last dimension
    # print(f"[PatchedTimeSeriesDecoder] config.patch_len = {config.patch_len}", end = '\n', flush=False)
  ...
  def _preprocess_input(
      ...
      use_patch_interpolate: int,
      linear_transform_patch_preprocess,
  ) -> tuple[
      torch.Tensor,
      torch.Tensor,
    ...
    # print(f"[PatchedTimeSeriesDecoder] config {self.config}, input_ff_layer.input_dims = {self.input_ff_layer.input_dims}", end = '\n', flush=False)

    ### qkcv
    # print(f"[PatchedTimeSeriesDecoder] patched_inputs.shape = {patched_inputs.shape}, patched_pads.shape = {patched_pads.shape}, concat_inputs.shape = {concat_inputs.shape}", end = '\n', flush=False) 

    if use_patch_interpolate==1:
      concat_inputs = torch.nn.functional.interpolate(concat_inputs, size=64, mode='linear', align_corners=True)
      model_input = self.input_ff_layer(concat_inputs)
    elif use_patch_interpolate==2:
      concat_inputs = linear_transform_patch_preprocess(concat_inputs)
      model_input = self.input_ff_layer(concat_inputs)
    elif use_patch_interpolate==3:
      # use linear_transform_patch_preprocess to transform the input
      model_input = linear_transform_patch_preprocess(concat_inputs)
  ...
  def _postprocess_output(
      ...
      use_patch_interpolate: int,
      linear_transform_patch_postprocess: torch.nn.Linear,
  ) -> torch.Tensor:
    """Postprocess output of stacked transformer."""

    # B x N x (H.Q)
    if use_patch_interpolate==3:
      output_ts = linear_transform_patch_postprocess(model_output)
    else:
    ...
    if use_patch_interpolate==1:
      ...
      output_ts = output_ts.view(b*n, num_outputs, -1)
      output_ts = torch.nn.functional.interpolate(output_ts, size=self.input_horizon_len, mode='linear', align_corners=True)
    elif use_patch_interpolate==2:
      output_ts = linear_transform_patch_postprocess(output_ts)
    # output_ts = output_ts.permute(0, 1, 3, 2)

    output_ts = output_ts.view(b, n, -1, num_outputs)
  ...
  def forward(
      ...
      use_patch_interpolate: int,
      linear_transform_patch_preprocess, 
      linear_transform_patch_postprocess,
      ca: torch.Tensor | None = None,
      v_qkcv: int = -1,
      input_horizon_len: int = 128,
  ) -> torch.Tensor:
    self.input_horizon_len = input_horizon_len
    ...
    model_input, patched_padding, stats, _ = self._preprocess_input(
        ...
        use_patch_interpolate=use_patch_interpolate,
        linear_transform_patch_preprocess=linear_transform_patch_preprocess,
    )
    f_emb = self.freq_emb(freq)  # B x 1 x D
    model_input += f_emb
    model_output, scores = self.stacked_transformer(model_input, patched_padding, ca=ca, v_qkcv=v_qkcv)

    output_ts = self._postprocess_output(model_output, num_outputs, stats, use_patch_interpolate, linear_transform_patch_postprocess)

    # print('\r', f'[PatchedTimeSeriesDecoder] forward, model_input.shape = {model_input.shape}, f_emb.shape = {f_emb.shape}, model_output.shape = {model_output.shape}, output_ts.shape = {output_ts.shape}', end = '\n', flush=True)
    #  [PatchedTimeSeriesDecoder] forward, model_input.shape = torch.Size([256, 8, 1280]), f_emb.shape = torch.Size([256, 1, 1280]), model_output.shape = torch.Size([256, 8, 1280]), output_ts.shape = torch.Size([256, 8, 128, 10])


    return output_ts, scores
